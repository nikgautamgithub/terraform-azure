                                ...
                                for key, value in resource.items():
                                    ...
                                    if key == "tags":
                                        if not value or value.strip().lower() == "none":
                                            # Assign an empty dictionary if tags are empty, None, or 'none'
                                            f.write(f'    {key} = {{}},\n')
                                        else:
                                            try:
                                                ...
                                                formatted_value = json.dumps(tags_dict, indent=2)
                                    ...
                                    if key in ["zones", "ports"]:
                                        if not value or value.strip().lower() == "none":
                                            # Assign an empty list if zones or ports are empty, None, or 'none'
                                            f.write(f'    {key} = [],\n')
                                        else:
                                            try:
                                    ...
                                    if key in ["data_disk_sizes", "data_disk_types"]:
                                        if not value or value.strip().lower() == "none":
                                            # Assign an empty list for these fields as well
                                            f.write(f'    {key} = [],\n')
                                        else:
                                            try:
                                                items = [v.strip() for v in re.split(r"[;,]", str(value)) if v.strip()]
                                                # Convert numerical fields appropriately
                                                if key == "data_disk_sizes":
                                                    # Assuming data_disk_sizes should be integers
